...
void inOrderTraversal(BSTNode *root)
{
	...
	// initialising stack as empty. will be initialised with 0xbaadf00d on s->top if not done so.
	s->top = NULL;
	push(s, root);
	...
	// if stack isn't empty and current node is null
	while (peek(s) != NULL && cur == NULL)
	{
		BSTNode *top = pop(s);
		printf("%d ", top->item);
		cur = top->right;
